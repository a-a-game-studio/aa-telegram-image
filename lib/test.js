...
const fProcessUrl = (fileSource) => {
    try {
        const aUrl = fileSource.split('/');
        let domain = aUrl[2];
...
const faLoadFileByAxios = async (sUrl) => {
    const userAgent = randomUseragent.getRandom((ua) => ua.browserName === 'Firefox');
    const respAxios = await axios_1.default.get(encodeURI(decodeURI(fProcessUrl(sUrl))), {
...
async function sendImg(photo) {
    ...
    try {
        const respAxios = await axios_1.default.post(`https://api.telegram.org/bot${bot.token}/sendPhoto`, data, {
            headers: {
                'Content-Type': 'application/json'
            }
        });
...
//faTestFnc();
index_1.faGetImg(bot.token, 'AgACAgQAAx0ETCGJ2QACAhhemeXIwZhtYdDLpogZ2epDSnhDSAACAasxG6h01FDQQ0zykJObCqK1BiNdAAMBAAMCAANtAAPigAACGAQ')
    .then(data => {
    console.log(data);
});
/*
